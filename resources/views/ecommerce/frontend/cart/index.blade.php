                                ...
                                <table>
                                    ...
                                    <tbody>
                                        ...
                                        @forelse ($keranjang as $item)
                                            ...
                                            @endif
                                            <tr>
                                                <td><input type="checkbox" name="cart" id="" class="btncheck"
                                                        @if ($item->check == 1)
                                                ...
                                                <td class="product-name" style="text-align: left">
                                                    ...
                                                    <div class="row">
                                                        @php
                                                            $ukuran = \AppHelper::instance()->ukuran($item->produk_id);
                                                        @endphp
                                                        <div class="col-md-12">
                                                            <a href="#" style="font-size: 11px; color:#8E8E93"
                                                                data-id="{{ $item->produk_id }}" class="wishlist-cart"><i
                                                                    class="ri-heart-add-line"></i> Tambahkan ke favorit</a>
                                                            <select name="ukuran" class="form-control ukuran">
                                                                @forelse ($ukuran as $row)
                                                                    <option value="{{ $row }}"
                                                                        data-id="{{ $item->id }}"
                                                                        @if ($row == $item->ukuran) selected @endif>{{ $row }}
                                                                    </option>
                                                                @empty
                                                ...
                                                <td class="product-price-cart amount">@rupiah($item->harga)
                                                </td>
                                                <td class="product-quantity pro-details-quality">
                                                    <div class="cart-plus-minus">

                                                        <input class="cart-plus-minus-box jumlah" type="text"
                                                            value="{{ $item->jumlah }}" data-id="{{ $item->id }}"
                                                            name="qtybutton" id="jumlah" value="1">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.ukuran').on('change', function() {
                ...
                var subtotal = $(this).closest("tr").find('.subtotal');
                var harga = $(this).closest("tr").find('.amount');
                ajax()
                $.ajax({
                    url: "{{ route('frontend.keranjang.update_ukuran') }}",
                    method: "POST",
                    data: {
                        id: id,
                        ukuran: ukuran,
            ...
            $('.wishlist-cart').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status) {
                            $('#data-alert-cart').html(
                                '<div class="alert alert-success" role="alert"> Produk berhasil di tambahkan ke favorit </div>'
                                )
                            setTimeout(function() {
                                $('#data-alert-cart').empty()
                            }, 1500)
                        }
                    }
                })
