            ...
            <div class="row">
                    ...
                    <div class="product-details-tab single-product-wrap">
                        <div class="product-dec-left-detail pro-dec-big-img-slider product-img product-img-zoom">
                            @forelse ($produk->detail_gambar as $item)
                                <img src="{{ asset('uploads/images/produk/' . $item->filename) }}" class="rounded-lg"
                                    alt="">
                            @empty
                            @endforelse
                        </div>
                        <div class="img-overlay">
                            <button class="btn btn-sm btn-wishlist rounded-circle wishlist" data-id="{{ $produk->id }}"
                                @if (auth()->check()) @if (\AppHelper::instance()->favorit_data(auth()->user()->id, $produk->id)) style="background-color:black;color:white" @endif
                                @endif><i class="ri-heart-line"></i></button>
                        </div>
                ...
                <div class="col-lg-6 col-md-12">
                    <div class="product-details-content pro-details-content-mt-md">
                        ...
                        <h5 class="mt-2">{{ "$produk->sub_kategori, $produk->kategori" }}</h5>
                        ...
                        @endif --}}
                            ...
                            @if ($produk->detail_produk->min('harga') == $produk->detail_produk->max('harga'))
                                <span
                                    class="new-price ml-2 hargaproduk">{{ \AppHelper::instance()->rupiah($produk->detail_produk->max('harga')) }}/seri</span>
                            @else
                                <span
                                    class="new-price ml-2 hargaproduk">{{ \AppHelper::instance()->rupiah($produk->detail_produk->min('harga')) }}
                                    -
                                    {{ \AppHelper::instance()->rupiah($produk->detail_produk->max('harga')) }}/seri</span>
                            @endif
                        ...
                        <div class="pro-details-quality">
                            ...
                            <div class="cart-plus-minus">
                                <input class="cart-plus-minus-box" type="text" name="qtybutton" id="jumlah"
                                    value="1">
                            </div>
                        </div>
                        <p>* 1 seri berisi 3 ukuran S, M, L (3 pcs).</p>
                    ...
                    <div id="des-details3" class="tab-pane">
                        ...
                        <div id="nav-selesai" role="tabpanel" aria-labelledby="nav-selesai-tab" class="tab-pane fade">
                            <div class="review-wrapper">
                                <h2>{{ \AppHelper::instance()->jumlah_ulasan($produk->id) }} ulasan untuk
                                    {{ $produk->nama_produk }}</h2>

                                @forelse ($produk->ulasan as $row)
                                    <div class="single-review">
                                        <div class="review-img">
                                            <img src="{{ asset('uploads/images/user/' . $row->user->foto) }}"
                                                style="width: 80%" alt="">
                                        </div>
                            ...
                            <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 col-6">
                                <div class="single-product-wrap mb-35 shadow rounded">
                                    <div class="product-img product-img-zoom mb-15">
                                        ...
                                        <div class="product-action-2 tooltip-style-2">
                                            <button title="Wishlist" data-id="{{ $item->id }}"
                                                class="wishlist-bottom"
                                                @if (auth()->check()) @if (\AppHelper::instance()->favorit_data(auth()->user()->id, $item->id)) style="background-color:black;color:white" @endif
                                                @endif ><i class="icon-heart"></i></button>
                                        </div>
                                    </div>
                                    <div class="product-content-wrap-2 text-left ml-2">

                                        <h3><a
                                                href="{{ route('frontend.product.show', [$item->id]) }}">{{ $item->nama_produk }}</a>
                                        </h3>
                                ...
                                @endif --}}
                                            @if ($item->detail_produk->min('harga') == $item->detail_produk->max('harga'))
                                                <span
                                                    class="new-price">{{ \AppHelper::instance()->rupiah($item->detail_produk->max('harga')) }}/seri</span>
                                            @else
                                                <span
                                                    class="new-price">{{ \AppHelper::instance()->rupiah($item->detail_produk->min('harga')) }}
                                                    -
                                                    {{ \AppHelper::instance()->rupiah($item->detail_produk->max('harga')) }}/seri</span>
                                            @endif
                                        </div>
