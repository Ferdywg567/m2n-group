                                                            ...
                                                            @if ($item->vendor == 'eksternal')
                                                                ...
                                                                @elseif(
                                                                    ...
                                                                    @if ($item->status_pembayaran == 'Belum Lunas')
                                                                        <a
                                                                            href="{{ route('pembayaran.create') }}?status=jahit&item={{ $item->id }}">
                                                                            <span
                                                                                class="badge badge-warning text-dark">{{ $item->status_pembayaran }}</span>
                                                                        </a>
                                                                    @else
                                                                        <a
                                                                            href="{{ route('pembayaran.edit', $item->pembayaran_jahit[0]) }}?status=jahit&item={{ $item->id }}">
                                                                            <span
                                                                                class="badge badge-warning text-dark">{{ $item->status_pembayaran }}</span>
                                                                        </a>
                                                                    @endif
...
@push('scripts')
    <script>
        $(document).ready(function() {
            ...
            let t = []
            t.push($('#tabelmasuk').DataTable({
                language: {
                    url: 'https://cdn.datatables.net/plug-ins/1.11.3/i18n/id.json'
                },
            }))
            t.push($('#tabelselesai').DataTable({
                language: {
                    url: 'https://cdn.datatables.net/plug-ins/1.11.3/i18n/id.json'
                },
            }))
