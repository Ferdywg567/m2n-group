...
<div id="non-printable">
                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                            data-target="#returMasuk">returan Masuk</a>
                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                            data-target="#returKeluar">returan Keluar</a>
                                    ...
                                    <table class="table table-hover" id="tabelbahanmasuk">
                                        ...
                                        <tbody id="">
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$item->no_surat}}</td>
                                                <td>
                                                    @if ($item->status_retur == 'belum retur')
                                                    <span
                                                        class="badge badge-secondary text-dark">{{$item->status_retur}}</span>
                                                    @elseif ($item->status_retur == 'selesai')
                                                    <span
                                                        class="badge badge-success text-dark">{{$item->status_retur}}</span>
                                                    @else
                                                    <span
                                                        class="badge badge-warning text-dark">{{$item->status_retur}}</span>
                                                    @endif
                                                </td>
                                                <td>
 ...
 --}}
    ...
    <div class="modal fade" id="returMasuk" tabindex="-1" role="dialog" aria-labelledby="returMasukLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="returMasukLabel">Input Data [retur Masuk]</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="formreturMasuk">
                    <div class="modal-body">
                        <div id="alert-retur-masuk">

                        </div>
                        <input type="hidden" name="status" value="retur masuk">
    ...
    <div class="modal fade" id="returKeluar" tabindex="-1" role="dialog" aria-labelledby="returKeluarLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="returKeluarLabel">Input Data [retur Keluar]</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="formreturKeluar">
                    <div class="modal-body">
                        <div id="alert-retur-keluar">

                        </div>
                        <input type="hidden" name="status" value="retur keluar">
...
<script>
    $(document).ready(function () {
             function ajax() {
              ...
              $('#returMasuk').on('hidden.bs.modal', function() {
                $(this).find('form').trigger('reset');
                $('#returMasukLabel').text('Input Data [retur Masuk]')
                $('#alert-retur-masuk').empty()
                $('.btnmasuk').prop('id','btnsimpanmasuk')
                $('.btnmasuk').show()
                $('#kdbahanselectmasuk').show()
              $('#kdbahanmasuk').hide()
              ...
              $('#returKeluar').on('hidden.bs.modal', function() {
                $(this).find('form').trigger('reset');
                $('#returKeluarLabel').text('Input Data [retur Keluar]')
                $('#alert-retur-keluar').empty()
                $('.btnkeluar').prop('id','btnsimpankeluar')
                $('#kdbahanselectkeluar').show()
                $('#kdbahankeluar').hide()
                $('.btnkeluar').show()
            ...
            $(document).on('click','#btnsimpanmasuk', function () {
                ...
                var form = $('#formreturMasuk').serialize()
                ajax()
                $.ajax({
                    ...
                    if(response.status){
                        $('#alert-retur-masuk').html(response.data)
                        setTimeout(function(){
                            $('#alert-retur-masuk').empty()
             ...
             $(document).on('click','.btneditmasuk',function () {
                    ...
                    $('#returMasukLabel').text('Edit Data [retur Masuk]')
                    $('#kdbahanselectmasuk').hide()
                    $('#returMasuk').modal('show')
                    $('#kdbahanmasuk').show()
                    $.ajax({
                        //
                        method:"GET",
            ...
            $(document).on('click','#btnupdatemasuk', function () {
             ...
             $(document).on('click','.btndetailmasuk',function () {
                    ...
                    $('.btnmasuk').hide()
                    $('#returMasukLabel').text('Detail Data [Bahan Masuk]')
                    $('#kdbahanselectmasuk').hide()
                    $('#returMasuk').modal('show')
                    $('#kdbahanmasuk').show()
            ...
            $(document).on('click','#btnsimpankeluar', function () {
                ...
                var form = $('#formreturKeluar').serialize()
                ajax()
                $.ajax({
                    url:"{{route('retur.store')}}",
                    method:"POST",
                    ...
                    if(response.status){
                        $('#alert-retur-keluar').html(response.data)
                        setTimeout(function(){
                            $('#alert-retur-keluar').empty()
                            location.reload(true)
                        },1500)
            ...
            $(document).on('click','.btneditkeluar',function () {
                    ...
                    $('#kdbahankeluar').show()
                    $('#returKeluarLabel').text('Edit Data [retur Keluar]')
                    $('#kode_bahan').prop('readonly', true)
                    $('#returKeluar').modal('show')
             ...
             $(document).on('click','.btndetailkeluar',function () {
                    ...
                    $('#kdbahanselectkeluar').hide()
                    $('#kdbahankeluar').show()
                    $('#returKeluarLabel').text('Detail Data [retur Keluar]')
                    $('#kode_bahan').prop('readonly', true)
                    $('#returKeluar').modal('show')
