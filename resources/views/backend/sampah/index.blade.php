...
<div id="non-printable">
                                ...
                                <div class="btn-group">
                                    ...
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                            data-target="#sampahMasuk">sampahan Masuk</a>
                                        <a class="dropdown-item" href="#" data-toggle="modal"
                                            data-target="#sampahKeluar">sampahan Keluar</a>
                                    ...
                                    <table class="table table-hover" id="tabelbahanmasuk">
                                        ...
                                        <tbody id="">
                                            ...
                                            <tr>
                                                ...
                                                <td>{{$item->no_surat}}</td>
                                                <td>
                                                    @if ($item->status_sampah == 'belum sampah')
                                                    <span
                                                        class="badge badge-secondary text-dark">{{$item->status_sampah}}</span>
                                                    @elseif ($item->status_sampah == 'selesai')
                                                    <span
                                                        class="badge badge-success text-dark">{{$item->status_sampah}}</span>
                                                    @else
                                                    <span
                                                        class="badge badge-warning text-dark">{{$item->status_sampah}}</span>
                                                    @endif
                                                </td>
                                                <td>
 ...
 --}}
    ...
    <div class="modal fade" id="sampahMasuk" tabindex="-1" role="dialog" aria-labelledby="sampahMasukLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="sampahMasukLabel">Input Data [sampah Masuk]</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="formsampahMasuk">
                    <div class="modal-body">
                        <div id="alert-sampah-masuk">

                        </div>
                        <input type="hidden" name="status" value="sampah masuk">
    ...
    <div class="modal fade" id="sampahKeluar" tabindex="-1" role="dialog" aria-labelledby="sampahKeluarLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="sampahKeluarLabel">Input Data [sampah Keluar]</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form id="formsampahKeluar">
                    <div class="modal-body">
                        <div id="alert-sampah-keluar">

                        </div>
                        <input type="hidden" name="status" value="sampah keluar">
...
<script>
    $(document).ready(function () {
             function ajax() {
              ...
              $('#sampahMasuk').on('hidden.bs.modal', function() {
                $(this).find('form').trigger('reset');
                $('#sampahMasukLabel').text('Input Data [sampah Masuk]')
                $('#alert-sampah-masuk').empty()
                $('.btnmasuk').prop('id','btnsimpanmasuk')
                $('.btnmasuk').show()
                $('#kdbahanselectmasuk').show()
              $('#kdbahanmasuk').hide()
              ...
              $('#sampahKeluar').on('hidden.bs.modal', function() {
                $(this).find('form').trigger('reset');
                $('#sampahKeluarLabel').text('Input Data [sampah Keluar]')
                $('#alert-sampah-keluar').empty()
                $('.btnkeluar').prop('id','btnsimpankeluar')
                $('#kdbahanselectkeluar').show()
                $('#kdbahankeluar').hide()
                $('.btnkeluar').show()
            ...
            $(document).on('click','#btnsimpanmasuk', function () {
                ...
                var form = $('#formsampahMasuk').serialize()
                ajax()
                $.ajax({
                    ...
                    if(response.status){
                        $('#alert-sampah-masuk').html(response.data)
                        setTimeout(function(){
                            $('#alert-sampah-masuk').empty()
             ...
             $(document).on('click','.btneditmasuk',function () {
                    ...
                    $('#sampahMasukLabel').text('Edit Data [sampah Masuk]')
                    $('#kdbahanselectmasuk').hide()
                    $('#sampahMasuk').modal('show')
                    $('#kdbahanmasuk').show()
                    $.ajax({
                        //
                        method:"GET",
            ...
            $(document).on('click','#btnupdatemasuk', function () {
             ...
             $(document).on('click','.btndetailmasuk',function () {
                    ...
                    $('.btnmasuk').hide()
                    $('#sampahMasukLabel').text('Detail Data [Bahan Masuk]')
                    $('#kdbahanselectmasuk').hide()
                    $('#sampahMasuk').modal('show')
                    $('#kdbahanmasuk').show()
            ...
            $(document).on('click','#btnsimpankeluar', function () {
                ...
                var form = $('#formsampahKeluar').serialize()
                ajax()
                $.ajax({
                    url:"{{route('sampah.store')}}",
                    method:"POST",
                    ...
                    if(response.status){
                        $('#alert-sampah-keluar').html(response.data)
                        setTimeout(function(){
                            $('#alert-sampah-keluar').empty()
                            location.reload(true)
                        },1500)
            ...
            $(document).on('click','.btneditkeluar',function () {
                    ...
                    $('#kdbahankeluar').show()
                    $('#sampahKeluarLabel').text('Edit Data [sampah Keluar]')
                    $('#kode_bahan').prop('readonly', true)
                    $('#sampahKeluar').modal('show')
             ...
             $(document).on('click','.btndetailkeluar',function () {
                    ...
                    $('#kdbahanselectkeluar').hide()
                    $('#kdbahankeluar').show()
                    $('#sampahKeluarLabel').text('Detail Data [sampah Keluar]')
                    $('#kode_bahan').prop('readonly', true)
                    $('#sampahKeluar').modal('show')
